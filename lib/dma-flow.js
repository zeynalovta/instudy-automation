...
export function createRegistrationUrl(userId) {
  const signature = createTrackingSignature(userId);

  return `${BASE_URL}/api/register?u=${encodeURIComponent(
    userId
  )}&s=${signature}`;
}
...
export async function startDmaFlow(userId, incomingText) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      last_incoming_text: incomingText,
...
export async function sendDmaFollowup(userId) {
  const registerUrl = createRegistrationUrl(userId);
  ...
  await sendButtonTemplate(
    ...
    [
      ...
      },
      {
        type: "postback",
        title: "Sualım var",
        payload: "DMA_HAVE_QUESTION"
      },
      {
        type: "postback",
        title: "Qeydiyyatdan keçdim",
        payload: "DMA_REGISTERED"
      }
    ]
  );
  ...
  const { error } = await supabase
    ...
    .update({
      followup_sent: true,
      last_action: "DMA_FOLLOWUP_SENT",
      last_action_at: new Date().toISOString(),
      current_step: "followup_sent"
    })
    .eq("instagram_user_id", userId);
